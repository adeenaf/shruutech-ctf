...
def submit_flag(challenge_id):
    ...
    status = "Correct" if submitted_flag == correct_flag else "Incorrect"
    ...
    if status == "Correct":
        ...
        session["total_score"] = total_score + challenge["points"]
...
def profile():
    ...
    if solved_ids:
        ...
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        query = f"SELECT id, title FROM challenges WHERE id IN ({','.join('?'*len(solved_ids))})"
        cursor.execute(query, solved_ids)
